...
// .wrangler/tmp/bundle-CYAvY6/checked-fetch.js
...
// .wrangler/tmp/bundle-CYAvY6/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-CYAvY6/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
init_checked_fetch();
init_modules_watch_stub();
var GLM46Client = class {
  static {
    __name(this, "GLM46Client");
  }
  config;
  constructor(config) {
    this.config = {
  ...
  async chat(request) {
    ...
    const requestBody = {
      ...
      max_tokens: request.max_tokens ?? 2e3,
  ...
  async analyzeBazi(baziInput, baziResult, language = "zh", aspects) {
...
\u8BF7\u4ECE\u6027\u683C\u7279\u5F81\u3001\u4E8B\u4E1A\u524D\u666F\u3001\u60C5\u611F\u5A5A\u59FB\u3001\u5065\u5EB7\u517B\u751F\u3001\u4EBA\u751F\u65F6\u673A\u7B49\u591A\u4E2A\u7EF4\u5EA6\u8FDB\u884C\u5168\u9762\u5206\u6790\uFF0C\u59CB\u7EC8\u5F3A\u8C03\u516B\u5B57\u662F\u8BA4\u8BC6\u81EA\u6211\u3001\u89C4\u5212\u4EBA\u751F\u7684\u5DE5\u5177\uFF0C\u800C\u975E\u4E0D\u53EF\u6539\u53D8\u7684\u547D\u8FD0\u5B89\u6392\u3002`;
  ...
  buildBaziPrompt(baziInput, baziResult, language, aspects) {
    if (language === "en") {
...
var GLM46Error = class extends Error {
  ...
  static {
    __name(this, "GLM46Error");
  }
};
function createGLM46Client() {
...
__name(createGLM46Client, "createGLM46Client");

// src/lib/ai.ts
async function analyzeBaziWithAI(baziInput, baziResult, language = "zh", aspects, provider = "deepseek") {
  try {
    let analysis;
    let providerName;
    if (provider === "glm46") {
...
async function compareAIAnalysis(baziInput, baziResult, language = "zh", aspects) {
...
__name(compareAIAnalysis, "compareAIAnalysis");
...
function parseAIResponse(content, language, provider = "DeepSeek") {
  return {
    analysis: content,
...
// src/lib/bazi.ts
async function analyzeBaziWithAI2(input, language = "zh", aspects) {
  const baziResult = calculateBaziResult(input);
  const provider = input.aiProvider || "deepseek";
  const aiAnalysis = await analyzeBaziWithAI2(
    input,
    baziResult,
    language,
    aspects,
...
__name(analyzeBaziWithAI2, "analyzeBaziWithAI");
async function analyzeBaziWithGLM46(input, language = "zh", aspects) {
  const baziResult = calculateBaziResult(input);
  const aiAnalysis = await analyzeBaziWithGLM46(
    input,
    baziResult,
    language,
    aspects
...
__name(analyzeBaziWithGLM46, "analyzeBaziWithGLM46");
async function compareBaziAnalysis(input, language = "zh", aspects) {
  const baziResult = calculateBaziResult(input);
...
__name(compareBaziAnalysis, "compareBaziAnalysis");
...
baziRoutes.post("/analyze", authMiddleware, async (c2) => {
  try {
    const userId = c2.get("userId");
    ...
    if (baziId) {
      ...
      return c2.json({ error: "Either baziId or baziInput is required" }, 400);
    }
    const result = await analyzeBaziWithAI2(input, language, aspects);
    if (baziId) {
    ...
    return c2.json({ error: "Failed to analyze bazi with AI" }, 500);
  }
});
baziRoutes.post("/analyze-glm46", authMiddleware, async (c2) => {
  try {
    const userId = c2.get("userId");
    const body = await c2.req.json();
    ...
    return c2.json({ error: "Failed to analyze bazi with GLM-4.6" }, 500);
  }
});
baziRoutes.post("/compare", authMiddleware, async (c2) => {
  try {
    const userId = c2.get("userId");
    const body = await c2.req.json();
    const { baziId, language = "zh", aspects, baziInput } = body;
    ...
    return c2.json({ error: "Failed to compare bazi analysis" }, 500);
  }
});
baziRoutes.post("/full-analysis", authMiddleware, async (c2) => {
  try {
    const userId = c2.get("userId");
    const body = await c2.req.json();
    const {
    ...
    if (!baziInput) {
      return c2.json({ error: "baziInput is required" }, 400);
    }
    if (!baziInput.year || !baziInput.month || !baziInput.day || !baziInput.hour) {
      return c2.json({ error: "Missing required date/time fields" }, 400);
    }
    ...
    if (saveToDb) {
      const prisma2 = getPrismaClient2(c2.env);
      savedData = await prisma2.baziData.create({
        data: {
          userId,
          inputData: input,
    ...
    return c2.json({
      ...
      createdAt: savedData?.createdAt || (/* @__PURE__ */ new Date()).toISOString()
    });
  } catch (error) {
    console.error("Full bazi analysis error:", error);
    return c2.json({ error: "Failed to perform full bazi analysis" }, 500);
  }
});
